...
def get_data_info(flags):
    if flags.dataset == 'jetnet150':
        ...
        for truth_path in truth_path_list:
            test_loader_list.append(utils.TruthTotalTauDataLoaderWithNpzForSample(truth_path,hvd.rank(),hvd.size(),nevts=10000,data_type='val'))
...
def load_data_and_model(flags):
    ...
    truth_path = flags.folder
    truth_path_list = glob.glob(truth_path + 'pi_pi/ml_export/*_1_recon.npz')
    test_loader_list = []
    for truth_path in truth_path_list:
        test_loader_list.append(utils.TruthTotalTauDataLoaderWithNpzForSample(truth_path,rank=hvd.rank(),size=hvd.size(),nevts=-1,data_type='val'))
    ...
    model_name = "/global/homes/b/baihong/workdir/data/NumpyData/checkpoints/PET_tautotalwithNpz_8_local_layer_scale_token_baseline_generator.weights.h5"
...
def main():
    ...
    if flags.sample:
        ...
        for i in range(len(test_path_list)):
            test_name = test_path_list[i].replace("_1_recon.npz", "_1_recon_eval.npz")
