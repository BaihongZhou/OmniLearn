...
from tensorflow.keras.losses import mse, categorical_crossentropy,huber
...
class PET(keras.Model):
    ...
    def __init__(self,
        ...
        self.regression_tracker = keras.metrics.RootMeanSquaredError(name="score")
    ...
    def metrics(self):
        ...
        if self.mode == 'regression':
            metrics.append(self.regression_tracker)
    ...
    def train_step(self, inputs):
        ...
        with tf.GradientTape(persistent=True) as tape:
            ...
            if self.mode == 'regression':
                body = self.body(x)
                y_pred,_ = self.classifier_head([body,x['input_jet']])
                loss = huber(y, y_pred)
        ...
        if self.mode == 'regression':
            trainable_vars = self.classifier_head.trainable_variables
            self.regression_tracker.update_state(y, y_pred)
