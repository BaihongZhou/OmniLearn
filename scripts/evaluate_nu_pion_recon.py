...
def get_data_info(flags):
    if flags.dataset == 'jetnet150':
        ...
        for truth_path in truth_path_list:
            test_loader_list.append(utils.RecoNuPionDataLoaderWithNpzForSample(truth_path,hvd.rank(),hvd.size(),nevts=10000,data_type='val'))
...
def load_data_and_model(flags):
    ...
    test_loader_list = []
    for truth_path in truth_path_list:
        test_loader_list.append(utils.RecoNuPionDataLoaderWithNpzForSample(truth_path,rank=hvd.rank(),size=hvd.size(),data_type='val'))
    ...
    model_name = "/pscratch/sd/b/baihong/data/checkpoints/PET_pipi_recon_pion_8_local_layer_scale_token_baseline_generator.weights.h5"
...
def sample_data(test, model, flags, sample_name):
    ...
    for i in range (total_j.shape[1]):
        total_jet.append(test.revert_preprocess_jet(total_j[:,i]).reshape(-1,1,14))
    ...
    if hvd.rank() == 0:
        logging.info("Saving the sampled data to {}.".format(sample_name))
        dict = {
            'nu_p':total_jet[:,:,:3],
            'nu_m':total_jet[:,:,7:10],
            'boosted_pion_p': total_jet[:,:,3:7],
            'boosted_pion_m': total_jet[:,:,10:],
            'EventID':EventID,
...
def main():
    ...
    if flags.sample:
        ...
        for i in range(len(test_path_list)):
            ...
            test_name = test_path_list[i].replace("_recon.npz", "_recon_eval_NP.npz")
