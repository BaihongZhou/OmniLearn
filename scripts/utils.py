...
class DataLoader:
    ...
    def __init__(self, path, batch_size=512, rank=0, size=1, keys='none',**kwargs):
    ...
    def make_eval_data(self):
        ...
        if self.keys == 'none':
            ...
            tf_zip = tf.data.Dataset.from_tensor_slices(
                {'input_features':X,
                'input_points':self.X_t[:,:,:2],
                'input_mask':self.mask.astype(np.float32),
                'input_jet':jet,
                'input_time':np.random.uniform(0, 1, (self.jet.shape[0],1)),
                'input_label': self.y})
    ...
    def load_data(self,path, batch_size=512,rank=0,size=1,nevts=None):
        ...
        self.X = h5.File(self.path,'r')['jets'][rank:nevts:size][:,:,:4]
        self.y_t = h5.File(self.path,'r')['regress'][rank:nevts:size]
        self.p_t = h5.File(self.path,'r')['fjets'][rank:nevts:size]
        self.lep = self.p_t[:,4:8]
        self.jet = self.y_t[:,10:]
        self.jet = self.get_pxyz(self.jet)
        self.X = np.concatenate([self.lep.reshape(-1,1,4),self.X],axis=1)
        print(self.X.shape)
        self.y = np.concatenate([self.p_t[:,14].reshape(-1,1),self.p_t[:,17].reshape(-1,1)],axis=1)
        ...
        if self.keys == 'none':
            pass
            # self.X = h5.File(self.path,'r')['jets'][rank:nevts:size]
            # self.y = h5.File(self.path,'r')['regress'][rank:nevts:size]
            # self.jet = h5.File(self.path,'r')['fjets'][rank:nevts:size]
            # self.y = self.y[:,10:]
            # print(self.y.shape)
            # self.jet = self.jet[:,]

        # else:
        #     # In this stage, X is the three momentum of nu, and y is the MET info.
        #     self.X = h5.File(self.path,'r')['regress'][rank:nevts:size]
        #     self.y_t = h5.File(self.path,'r')['fjets'][rank:nevts:size]
        #     self.jet = h5.File(self.path,'r')['jets'][rank:nevts:size]
        #     self.X_t = self.X[:,10:]
        #     self.y = np.concatenate([self.y_t[:,14].reshape(-1,1),self.y_t[:,17].reshape(-1,1)],axis=1)
        #     self.X = self.get_pxyz(self.X_t)
        #     self.X_t = self.X_t.reshape(-1,1,4)
        #     self.jet = self.jet[:,:,0:4]
        #     self.jet = np.concatenate([self.y_t[:,4:8].reshape(-1,1,4),self.jet],axis=1)
        #     self.jet = self.jet.reshape(self.jet.shape[0], -1)
        #     self.mask = self.X_t[:,:,2]!=0
...
class TopDataLoaderWithGenerator(DataLoader):    
    def __init__(self, path, batch_size=512,rank=0,size=1,keys='generated'):
        super().__init__(path, batch_size, rank, size)
