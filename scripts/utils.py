...
class DataLoader:
    ...
    def load_data(self,path, batch_size=512,rank=0,size=1,nevts=None):
        ...
        self.y = h5.File(self.path,'r')['fjets'][rank:nevts:size]
        self.jet = h5.File(self.path,'r')['regress'][rank:nevts:size]
        self.y = self.y[:,10:]
        self.y = self.get_pxyz(self.y)
        self.jet = self.jet[:,4:8]
...
class TopDataLoaderWithGenerator(DataLoader):    
    def __init__(self, path, batch_size=512,rank=0,size=1,keys='generated'):
        ...
        self.load_data(path, batch_size,rank,size)
        self.num_pad = 6
        self.num_feat = self.X.shape[1] + self.num_pad #missing inputs
