...
#     jet_1_temp = np.stack([jet_1.px, jet_1.py, jet_1.pz, jet_1.E, np.zeros_like(jet_1.pt), np.zeros_like(jet_1.pt),
#                            np.zeros_like(jet_1.pt), np.zeros_like(jet_1.pt), np.zeros_like(jet_1.pt)], -1)
#     jet_2_temp = np.stack([jet_2.px, jet_2.py, jet_2.pz, jet_2.E, np.zeros_like(jet_2.pt), np.zeros_like(jet_2.pt),
#                            np.zeros_like(jet_2.pt), np.zeros_like(jet_2.pt), np.zeros_like(jet_2.pt)], -1)
#     jet_3_temp = np.stack([jet_3.px, jet_3.py, jet_3.pz, jet_3.E, np.zeros_like(jet_3.pt), np.zeros_like(jet_3.pt),
#                            np.zeros_like(jet_3.pt), np.zeros_like(jet_3.pt), np.zeros_like(jet_3.pt)], -1)
#     MET_temp = np.stack([MET.pt, MET.phi], -1)
#     tau_p_child1_temp = np.stack(
#         [tau_p_child1.px, tau_p_child1.py, tau_p_child1.pz, tau_p_child1.E, tau_p_child1_charge, tau_p_child1_is_el,
#          tau_p_child1_is_mu, tau_p_child1_is_charged_pion, tau_p_child1_is_neutral_part], -1)
#     tau_p_child2_temp = np.stack(
#         [tau_p_child2.px, tau_p_child2.py, tau_p_child2.pz, tau_p_child2.E, tau_p_child2_charge, tau_p_child2_is_el,
#          tau_p_child2_is_mu, tau_p_child2_is_charged_pion, tau_p_child2_is_neutral_part], -1)
#     tau_m_child1_temp = np.stack(
#         [tau_m_child1.px, tau_m_child1.py, tau_m_child1.pz, tau_m_child1.E, tau_m_child1_charge, tau_m_child1_is_el,
#          tau_m_child1_is_mu, tau_m_child1_is_charged_pion, tau_m_child1_is_neutral_part], -1)
#     tau_m_child2_temp = np.stack(
#         [tau_m_child2.px, tau_m_child2.py, tau_m_child2.pz, tau_m_child2.E, tau_m_child2_charge, tau_m_child2_is_el,
#          tau_m_child2_is_mu, tau_m_child2_is_charged_pion, tau_m_child2_is_neutral_part], -1)
...
#     X = np.concatenate((tau_p_child1.reshape(tau_p_child1.shape[0], 1, 9),
#                         tau_p_child2.reshape(tau_p_child1.shape[0], 1, 9),
#                         tau_m_child1.reshape(tau_p_child1.shape[0], 1, 9),
#                         tau_m_child2.reshape(tau_p_child1.shape[0], 1, 9)), axis=1)
#     X = np.concatenate((X, jet_1.reshape(jet_1.shape[0], 1, 9), jet_2.reshape(jet_2.shape[0], 1, 9),
#                         jet_3.reshape(jet_3.shape[0], 1, 9)), axis=1)
...
def process(data_path: Path, save_path: Path, sample_lists: list[str], features: dict, train_ratio: float = 0.8):
    kin_data = {}
    extra_data = {}
        ...
        with open(data_path / file_path, 'rb') as f:
            ...
            for category, attributes in features.items():
                ...
                for particle in attributes.get('particles', []):
                    ...
                    particle_feature = []
                    for kin in attributes.get('kinematic', []):
                        particle_feature.append(kin_par[kin][:, None])

                    # Fill in Extra
                    for extra in attributes.get('extra', []):
                        extra_par = np.zeros_like(pt)
                        if category == 'tau_vis':
                            extra_par = data[particle + '_' + extra]
                        particle_feature.append(extra_par[:, None])

                    # Fill in label
                    if category == 'tau_vis':
                        particle_feature.append(np.ones_like(pt[:, None]) * attributes.get('label', 1))
                    if category == 'jet':
                        particle_feature.append(np.ones_like(pt[:, None]) * attributes.get('label', 0))

                    X.setdefault(particle, []).append(np.concatenate(particle_feature, axis=1))

    X = {p_name: np.concatenate(p)[:, None, :] for p_name, p in X.items()}
    X = np.concatenate(list(X.values()), axis=1)
...
def main():
    ...
    process(data_path=data_path, save_path=save_path, sample_lists=sample_lists, features=features, train_ratio=0.8)
